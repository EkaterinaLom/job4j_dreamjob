...
public class Sql2oCandidateRepositoryTest {
    ...
    public void whenUpdateThenGetUpdated() {
        ...
        var savedCanndidate = sql2oCandidateRepository.findByID(updatedCandidate.getId()).get();
        assertThat(isUpdated).isTrue();
        assertThat(savedCanndidate).usingRecursiveComparison().isEqualTo(updatedCandidate);
    }

    @Test
    public void whenUpdateUnExistingCandidateThenGetFalse() {
        var creationDate = now().truncatedTo(ChronoUnit.MINUTES);
        var candidate = new Candidate(0, "name", "description", creationDate, 1,
                file.getId());
